...
export default function DashboardHome() {
  ...
  const recentTransactions = useMemo(() => {
    return transactions
      .map((tx) => ({
        ...tx,
        farmerName: farmers.find((f) => f.id === tx.farmerId)?.name || "—",
        location: farmers.find((f) => f.id === tx.farmerId)?.location || "—",
      }))
      ...
      <div className="bg-[#0F3D21] rounded-2xl p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        <div>
          ...
          <p className="text-white/70 text-sm max-w-lg leading-relaxed">
            Here is your financial and producer overview for Santo Tomas buying
            stations and partner farm lots.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-gray-100 flex flex-col">
          ...
          <div className="mt-auto">
            ...
            <div className="text-xs text-[#0F3D21] font-bold mt-1 flex items-center gap-1">
              of {farmers.length} total farmers
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#154226] p-6 rounded-2xl shadow-sm border border-[#0F3D21] flex flex-col text-white">
          ...
          <div className="mt-auto">
            ...
            <div className="text-xs text-[#a0d2b4] font-bold mt-1">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-gray-100 flex flex-col">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <button
              ...
              className="text-xs font-bold text-[#0F3D21] hover:underline"
            >
              View All
            </button>
          </div>
                ...
                {recentTransactions.map((tx) => (
                  ...
                  >
                    <td className="px-6 py-4 text-gray-500 text-xs">
                      {format(new Date(tx.date), "MMM dd, h:mm a")}
                    </td>
                    <td className="px-6 py-4 font-bold text-gray-900 line-clamp-1">
                      {tx.farmerName}
                    </td>
          ...
          <div className="p-4 border-t border-gray-100 mt-auto text-center">
            <span className="text-xs text-gray-400">
              Showing latest {recentTransactions.length} of{" "}
                  ...
                  return (
                    <div key={item.id}>
                      <div className="flex justify-between items-end mb-2">
                        ...
                        <span
                          className={`text-[10px] font-bold ${isCritical ? "text-red-500" : "text-[#dfa43a]"}`}
                        >
                          {item.stock} left
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between text-[9px]">
                        <span className="text-gray-400">
                          Safety min: {item.reorderLevel}
                        </span>
                        <span
                          className={`font-bold ${isCritical ? "text-red-500" : "text-[#dfa43a]"}`}
                        >
